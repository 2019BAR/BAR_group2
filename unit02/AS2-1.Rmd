...
```{r}
a = as.POSIXct(D$Date, format="%m/%d/%y %H:%M")
par(cex=0.7)
hist(a,"year",las=2,freq=T,xlab="")
#
```
...
```{r}
table(a > as.POSIXct("2007-01-01"))
#
```

【3.3】 For what proportion of motor vehicle thefts in 2001 was an arrest made?
```{r}
table(D$Arrest, format(a,'%Y')) %>% prop.table(2) %>% round(3)
#
```
...
tapply(D$Arrest, format(a,'%Y'), mean) %>% round(3)
```
