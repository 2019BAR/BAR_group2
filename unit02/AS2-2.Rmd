...
nrow(IBM)
#There're 480 observations in each data set.
```
...
CO$Date[which.max(CO$StockPrice)]
CO$Date[which.min(CO$StockPrice)]
abline(v=c(CO$Date[which.max(CO$StockPrice)],CO$Date[which.min(CO$StockPrice)]),h=c(max(CO$StockPrice),min(CO$StockPrice)), col='red')
#highest stock price:1973-01-01
#lowest stock price:1980-03-01
```
...
```{r}
#The stock of Procter and Gamble was going down.
#The stock of Coca-Cola was going up.
```
...
lines(CO$Date, CO$StockPrice, type='l', col='green')
lines(PG$Date, PG$StockPrice, type='l', col='blue')
#IBM's stock reaches the highest value in the time period 1995-2005.
```
...
which.max(tapply(IBM$StockPrice, format(IBM$Date,'%m'), mean))
#February
```
...
=======
>>>>>>> 6788560c5ff34079ee91c1a4312a15c55d9fb2af
